...
const typesCategories = (currentCategorie, setCurrentView, setCurrentType) => {
	if (currentCategorie === "ECONÓMICO") {
		return tiposEconomico.map((indicador, index) => {
			return (
		...
		return tiposSocial.map((indicador, index) => {
			return (
				<Grid item xs={12} sm={6} md={4} lg={4} key={index}>
					<IndicadorTipo
						nombre={indicador.nombre}
		...
		return tiposAmbiental.map((indicador, index) => {
			return (
				<Grid item xs={12} sm={6} md={4} lg={4} key={index}>
					<IndicadorTipo
						nombre={indicador.nombre}
...
function Main({ currentView, setCurrentView }) {
	...
	const handleCurrentIndicator = (selectedOption) => {
		setCurrentIndicator(filteredIndicators[selectedOption]);
	};
	...
	const getIndicators = useMemo(
		() => async () => {
			try {
				const url = indicatorRoutes.getIndicators;
				console.log("url", url);
				axios
					.get(url, {
						headers: {
					...
					.then((response) => {
						...
						setFilteredIndicators(response.data.indicators);
					})
					.catch((error) => {
						console.log(error);
					});
	...
	useEffect(() => {
		let filtered = indicators.filter((indicator) => {
			indicator.sourceType = indicator.sourceType.toLowerCase();
			if (currentType.length > 0) {
				return (
					indicator.categorie.toLowerCase() ===
						currentCategorie.toLowerCase() &&
					currentType.includes(indicator.sourceType)
				);
			} else {
				return (
					indicator.categorie.toLowerCase() ===
	...
	if (currentView == 0) {
		...
		return (
			<Grid container rowSpacing={1}>
				<Grid item xs={12}>
					<Filters
						currentCategorie={currentCategorie}
						tiposAmbiental={tiposAmbiental}
						tiposEconomico={tiposEconomico}
						tiposSocial={tiposSocial}
						currentType={currentType}
						setCurrentType={setCurrentType}
						addCurrentType={addCurrentType}
		...
		return <LastView currentIndicator={currentIndicator} />;
	}
}
