...
function AppLayout({ noHeader, animateClass, companyInfo }) {
	...
	return (
		<div
			className={`animate__animated ${animateClass} grid lg:grid-cols-11 xl:grid-cols-12 h-screen w-screen`}
		>
			<Sidebar theme={theme} />
			<header className="lg:col-span-10 xl:col-span-11 overflow-y-scroll">
				{!noHeader && (
					<Header currentTheme={theme} companyInfo={companyInfo} />
				)}
				<Outlet />
			</header>
		</div>
	...
	return (
		<Routes>
			...
			>
				<Route
					index
					element={<Dashboard companyInfo={companyInfo} />}
				/>
				<Route
					path="perfil"
					element={<Setting companyInfo={companyInfo} />}
				/>{" "}
				...
				<Route
					path="ayuda"
					element={<Ayuda companyInfo={companyInfo} />}
				/>
			</Route>
...
function App() {
	return (
		<Router>
			<Routes>
				<Route
					path="/reset-password/:recoveryPassToken"
